...
namespace kOSMainframe.VesselExtra
{
    ...
    public class FuelNode
    {
        ...
        private void Init(Part part, bool dVLinearThrust)
        {
            ...
            if (engine != null)
            {
                ...
                if ((HighLogic.LoadedSceneIsEditor || inverseStage < StageManager.CurrentStage || engine.getIgnitionState) && (engine.thrustPercentage > 0 || engine.minThrust > 0))
                {
                    ...
                    propellantSumRatioTimesDensity = engine.propellants.Slinq().Select(prop => prop.ratio * ResourceContainer.GetResourceDensity(prop.id)).Sum();
                    float ratio = propellantSumRatioTimesDensity / engine.propellants.Slinq().Where(prop => !prop.ignoreForIsp).Select(prop => prop.ratio * ResourceContainer.GetResourceDensity(prop.id)).Sum();
                    maxFuelFlow *= ratio;
                    minFuelFlow *= ratio;
                    propellantRatios.Clear();
                    propellantFlows.Clear();
        ...
        public double Mass(int simStage)
        {
            ...
            double resMass = resources.KeysList.Slinq().Select((r, rs) => rs[r] * ResourceContainer.GetResourceDensity(r), resources).Sum();
