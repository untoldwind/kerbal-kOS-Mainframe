...
namespace kOSMainframe.Simulation {
    public class SimulatedVessel {
        ...
        public static SimulatedVessel Borrow(Vessel v, SimCurves simCurves, double startTime, int limitChutesStage) {
            SimulatedVessel vessel = pool.Borrow();
            vessel.Init(v, simCurves, startTime, limitChutesStage);
            return vessel;
        }

        private void Init(Vessel v, SimCurves _simCurves, double startTime, int limitChutesStage) {
            totalMass = 0;
